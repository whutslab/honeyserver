...
class Fingerprint():
    ...
    MONGO_ATTRIBUTES = [COUNTER, ID, CREATION_TIME, FONTS_FLASH, LANGUAGE_HTTP, RENDERER, RESOLUTION_JS, PLUGINS_JS,
                        USER_AGENT_HTTP,PLATFORM_JS,CANVAS_JS_HASHED, DNT_JS, COOKIES_JS]
    ...
    @classmethod
    def unknownfp(cls,list_attributes,val_attributes):
        fp=cls.__new__(cls)
        fp.val_attributes = dict()
        for attribute in list_attributes:
            try:
                fp.val_attributes[attribute] = val_attributes[attribute]
            except:
                fp.val_attributes[attribute] = None
        parsedUa = user_agent_parser.Parse(val_attributes[Fingerprint.USER_AGENT_HTTP])
        fp.val_attributes[Fingerprint.BROWSER_FAMILY] = parsedUa["user_agent"]["family"]
        fp.val_attributes[Fingerprint.OS] = parsedUa["os"]["family"]
        fp.constant_hash = hashlib.md5(str.encode(fp.val_attributes[Fingerprint.OS] + \
                                                    fp.val_attributes[Fingerprint.PLATFORM_JS] + \
                                                    fp.val_attributes[Fingerprint.BROWSER_FAMILY]
                                                    )
                                         ).hexdigest()
        fp.val_attributes[Fingerprint.FONTS_FLASH_HASHED] = hashlib.sha1(str.encode(fp.FONTS_FLASH))
        fp_string = []
        fp_string += fp.val_attributes[Fingerprint.ID]
        fp_string += fp.val_attributes[Fingerprint.PLUGINS_JS]
        fp_string += fp.val_attributes[Fingerprint.PLATFORM_JS]
        fp_string += fp.val_attributes[Fingerprint.RESOLUTION_JS]
        fp_string += fp.val_attributes[Fingerprint.CANVAS_JS_HASHED]
        fp_string += fp.val_attributes[Fingerprint.USER_AGENT_HTTP]
        fp_string += fp.val_attributes[Fingerprint.LANGUAGE_HTTP]
        fp_string += fp.val_attributes[Fingerprint.FONTS_FLASH]
        fp.hash = hashlib.md5(str.encode("".join(fp_string))).hexdigest()
        return fp
